...
const defaultItems = [
	{
		...
		side: "left",
		skills: ["Team Leadership", "Writing & Communication", "Project Ownership", "Time Management", "Public Speaking"],
	...
	},
	{
		...
		side: "right",
		skills: ["Photography & Composition", "Color Management", "Studio & Natural Lighting", "Content Workflow"],
	...
	},
	{
		...
		side: "left",
		skills: ["Client Discovery", "Creative Direction", "Branding & Marketing", "Operations Management", "Customer Retention"],
		techTools: ["Adobe Illustrator", "Adobe InDesign", "MailChimp", "Squarespace", "Google Analytics", "WordPress"],
	},
	{
		id: 4,
		year: "2017",
		...
		side: "right",
		skills: ["Customer Experience", "Property CMS", "Scheduling & Coordination", "Process Improvement", "Stakeholder Communication"],
	...
	},
	{
		...
		side: "left",
		skills: ["Talent Sourcing", "Interview Facilitation", "Pipeline Management", "Hiring Metrics", "Diversity Initiatives"],
	...
	},
	{
		...
		techTools: ["JavaScript", "React", "Node.js", "Next.js", "Express", "HTML5", "CSS3", "Git", "GitHub", "MongoDB"],
	},
	{
		id: 7,
		year: "2024",
		...
		side: "left",
		skills: ["UX Engineering", "Product Development", "Design Systems", "Data-informed UX", "Cross-functional Collaboration", "Regulated Environments"],
		techTools: ["React", "TypeScript", "JavaScript", "Figma", "Articulate 360", "Adobe Suite", "GSAP", "Microsoft Sharepoint"],
	},
	{
		id: 8,
		year: "2026",
		title: "Johnson & Wales University · Food Studies, Media & Design",
		text: "I don’t believe in silos. At Johnson & Wales, I’m studying how food, media, and design shape culture and community while continuing to build on my background in UX and front-end development. My work has included enterprise products, design systems, data-heavy interfaces, and inclusive user experiences. Different subjects, same instinct: understand the system, find what matters, and make it clearer.",
		side: "right",
		skills: ["User-Centered Design", "Research & Case Studies", "Media Storytelling", "Food Systems & Culture", "Project Delivery"],
		techTools: ["Figma", "Adobe Creative Cloud", "Plasmic", "Elementor", "Webflow", "Microsoft CoPilot", "Tailwind CSS"],
	},
];
				...
				try {
					displayItems.forEach((item, i) => {
						try {
							...
							if (card) {
								gsap.fromTo(card, { opacity: 0, y: 30, willChange: "opacity, transform" }, { opacity: 1, y: 0, duration: 0.6, scrollTrigger: { trigger: card, start: "top 85%", toggleActions: "play none none reverse", fastScrollEnd: true }, clearProps: "willChange" });
							}
							if (badge) {
								gsap.fromTo(badge, { scale: 0, opacity: 0, willChange: "transform, opacity" }, { scale: 1, opacity: 1, duration: 0.4, ease: "back.out(1.7)", scrollTrigger: { trigger: badge, start: "top 90%", toggleActions: "play none none reverse", fastScrollEnd: true }, clearProps: "willChange" });
							}
							if (opposite) {
								gsap.fromTo(opposite, { opacity: 0, x: item.side === "right" ? -30 : 30, willChange: "opacity, transform" }, { opacity: 1, x: 0, duration: 0.6, scrollTrigger: { trigger: opposite, start: "top 85%", toggleActions: "play none none reverse", fastScrollEnd: true }, clearProps: "willChange" });
							}
						} catch (err) {
	...
	return (
		<section className="timeline-section" ref={containerRef}>
			<div className="timeline-header">
				...
				<a href="/Resume_DeCoteauSam2026.pdf" download="Resume_DeCoteauSam2026.pdf" className="resume-download-btn" aria-label="Download Resume">
				...
				{displayItems.map((item, i) => {
					...
					return (
						<div key={item.id || i} className={`timeline-item ${side}`}>
							<div className="timeline-badge" ref={(el) => (badgesRef.current[i] = el)} style={{ borderColor: accent, backgroundColor: accent }}>
								<span>{item.year}</span>
							</div>
							<article className="timeline-card" ref={(el) => (cardsRef.current[i] = el)} style={{ borderColor: accent }}>
								<h3 className="timeline-title">{item.title}</h3>
								<p className="timeline-text">{item.text}</p>
							</article>
							{(item.skills || item.techTools || item.images) && (
								<div className={`timeline-opposite ${oppositeSide}`} ref={(el) => (oppositeRef.current[i] = el)}>
									{item.skills && <ul className="skills-list">{item.skills.map((skill, idx) => <li key={idx}>{skill}</li>)}</ul>}
									{item.techTools && <div className="tech-tools">{item.techTools.map((tool, idx) => <span key={idx} className="tech-tool-pill">{tool}</span>)}</div>}
									{item.images && <div className="project-mosaic">{item.images.map((img, idx) => <img key={idx} src={img.src} alt={img.alt || `Project image ${idx + 1}`} className="project-image" />)}</div>}
								</div>
							)}
						</div>
					);
